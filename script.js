...
try{
  if(schedVideo) schedVideo.volume = 0.4;
  // also set any other video elements on the page
  document.querySelectorAll('video').forEach(v=>{ try{ v.volume = 0.4 }catch(e){} });
...
function showResults(result){
  ...
  const blocks = Array.from(document.querySelectorAll('.process-block'));
  // Only show waiting times for processes that had a positive burst value.
  // For blank/zero-burst rows, leave the waiting display as a dash to avoid confusion
  // and exclude them from the AWT calculation.
  blocks.forEach(b=>{
    const name = b.querySelector('.label').textContent.trim();
    const proc = procs.find(p => p.name === name);
    const waitEl = b.querySelector('.waiting-display');
    if (proc && proc.burst > 0) {
      waitEl.textContent = proc.waiting;
    } else {
      // keep blank/unused rows visually empty (use em-dash)
      waitEl.textContent = '—';
    }
  });

  // Compute AWT only over processes with burst > 0
  const validProcs = procs.filter(p => p.burst > 0);
  const totalWaiting = validProcs.reduce((s,p)=>s+p.waiting,0);
  const awt = validProcs.length ? (totalWaiting / validProcs.length) : 0;
  document.getElementById('awtValue').textContent = validProcs.length ? awt.toFixed(2) : '—';
